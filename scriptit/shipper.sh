...
#### -@Usage:   bash shipper.sh -[-pass/i] <password / identity file> 
####            user@<machine> <script> --port <portnumer def:22>
#### -@Example: bash shipper.sh --pass my_password user@192.168.1.1 
####            script
#### -@Example: bash shipper.sh -i my_key.pem user@192.168.1.1 script
...
#Build expect file to copy shit.sh using password
...
#log into machine and execute shit.sh for pass
...
#log into machine and execute shit.sh for key
...
then
    ...
    expect $DIR'/'expexecpass.exp "$MACHINE" "$PWDORKEY" "$PORT" "$FILE"
    echo ""
    echo "**************         \"DONE\"          *****************"
...
then
    ...
    echo ">File copied"
    echo ">Executing $FILE from local machine to $MACHINE using port number $PORT..."
    expect $DIR'/'expexeckey.exp "$MACHINE" "$PWDORKEY" "$PORT" "$FILE"
    echo ""
    echo "**************         \"DONE\"          *****************"
